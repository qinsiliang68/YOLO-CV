...
The study covers five YOLO11 classification backbones (n/s/m/l/x) under a shared training protocol and is organized in three parts. First, we establish a gate-aligned evaluation protocol combining temperature scaling, recall-constrained threshold search, per-epoch external evaluation, and an extended validation set, reducing the 95\% Wilson confidence interval of specificity estimates from $\pm$10pp (under small-sample evaluation) to within $\pm$0.85pp. Second, we conduct full hard-negative (HN) ratio sweeps at 11 points on each capacity tier, producing 55 independent training runs to quantify the \textbf{capacity-dependent} nature of HN replay gains. Third, we propose a sample valuation framework built on four orthogonal signals---boundary risk ($R$), neighborhood density ($D$), training dynamics ($T$), and consistency under perturbation ($C$)---and verify the Goldilocks effect (that mid-range samples carry higher training value than extreme samples) across multiple independent signals via quintile-stratified ablation and sliding-window experiments.
...
\end{equation}
其中 $\theta$ 为模型参数与工作阈值的联合决策变量，$r_0$ 为预设召回下界（本文取 $r_0 \in \{99.5\%,\;99.0\%\}$），$\mathrm{Spec}$ 为正常样本拦截率。针对第一处不足，式~\eqref{eq:gate-objective} 显式将召回约束写入优化目标，取代默认阈值分类语义；针对第二处不足，本文构建约 3{,}800 正常样本的工作点验证集（val-op）与约 13{,}400 正常样本的独立测试集，将 Wilson 95\% 置信区间宽度从旧 84 样本的 $\pm$10pp 压缩至 $\pm$0.85pp；针对第三处不足，本文建立由温度校准\cite{guo2017calibration}、召回约束阈值搜索\cite{elkan2001foundations,kumar2021constrained}、逐 epoch 外部评估构成的评价协议，并在该协议下对难负样本回流与四类正交样本价值信号（边界风险、邻域密度、训练动态、扰动一致性）进行定量刻画，系统研究\textbf{二分类门控任务中高价值样本的评估方法}。
...
\subsection{技术路线}
本文的技术路线由四个递进阶段组成。\textbf{第一阶段（等条件容量扫描）}：在直接二分类门控视图上执行五容量（n/s/m/l/x）扫描，通过温度校准与外部评估器确定骨干网络排序。\textbf{第二阶段（HN 比例全量扫描）}：在五个容量上各自完成 $r \in \{0\%,2\%,\ldots,20\%\}$ 共 11 个比例点的全量扫描（hard-negative ratio sweep），共计 55 个独立训练 run，刻画 HN 增益的容量依赖性与形态异质性。\textbf{第三阶段（样本价值信号分解与 Goldilocks 验证）}：将候选池按 $R$/$D$/$T$/$C$ 四信号各自排序后均分为 5 个分位桶或 11 个滑动窗口，执行分层消融实验，验证各信号的独立排序力与非单调价值响应。\textbf{第四阶段（扩展验证集重评）}：构建约 3{,}800 正常样本的工作点验证集（val-op）与约 13{,}400 正常样本的独立测试集，对全部 55 个 checkpoint 做 prior-agnostic 重新评估，暴露既有小样本评估的乐观偏差。
...
\subsection{采样流程}
本文采用随机采样（uniform random sampling）从采样池中抽取训练、验证与测试集，\textbf{采样种子固定}为 20260606，保证数据集构建可重现。采样不是一次性整体抽取，而是按训练侧与评估侧分别遵循不同原则的两步：

\textbf{训练集（24{,}000 张）的构成}。训练集需服务于二分类门控任务，其类别先验由研究人员主动设定为 1:1 平衡（详见 \ref{sec:split-sizes}），具体分解为 12{,}000 张缺陷帧 + 12{,}000 张正常帧。缺陷部分按"每类 2{,}000 张"配额（PF/DE/FS/RB/AF/OB 六类合 12{,}000）从采样池中抽取；考虑多标签共现，本文采用\textbf{rarity-priority} 消歧机制：对每张多标签帧按"稀有类优先"顺序 PF $\succ$ DE $\succ$ RB $\succ$ AF $\succ$ OB $\succ$ FS 唯一归类一次，从而避免高频缺陷在配额中重复占位、压制稀有类的实际入选数量。该顺序由 Sewer-ML 训练池的标注频率统计反向排序确定（PF、DE 为最稀有类）。正常部分则从 Defect=0 帧池中独立无放回抽取 12{,}000 张。

\textbf{验证集与测试集（合计 28{,}000 张）的构成}。验证与测试集服务于校准、阈值搜索与最终报告，其类别分布需\textbf{忠实反映真实部署场景}（详见 \ref{sec:eval-impl}），故不做 1:1 平衡，而采用与采样池一致的自然分布（正常帧约 67\%，缺陷帧约 33\%）。本文这部分子集直接\textbf{沿用前期已生成的 v1 自然分布划分}：val-cal（2{,}400 张）、val-op（5{,}600 张）、test（20{,}000 张）三个子集与 v1 版本字节级一致，不重新抽样，确保前后两版数据集的评估结果在评估侧完全可比。

\textbf{两步采样的合理性}。训练侧主动干预（1:1 平衡）与评估侧被动观察（自然分布）的分离，是本文数据集构建的核心设计取舍——前者避免类不平衡导致的训练梯度偏向正常类，后者使 Spec@R$_0$ 等工作点指标的分母与部署现场的样本流量匹配，二者在角色上不冲突。\ref{sec:split-sizes} 节将就该取舍展开规模决策的下界—上界分析。

\textbf{后续目标检测任务的数据预留}。本文聚焦二分类门控（第一阶段），但同一采样池亦支持后续的目标检测任务（第二阶段，留作未来工作）。届时可\textbf{复用同一 12{,}000 张缺陷帧}并配以 4{,}000 张正常帧子集（约 3:1 缺陷:正常），以适配检测任务的样本预算特征；该预留不影响本文数据集的任何使用方式，但保证两阶段任务共享同一缺陷样本集，避免缺陷帧在两阶段间因独立抽取而引入的人工差异。
...
\textbf{约束三：计算预算的上界}。本文需在五种容量上分别完成训练（第 4 章），后续章节还需在同一协议下开展 HN 比例扫描与 RDTC 信号消融，累计训练 run 数在数十至上百量级。单次训练耗时近似与 $n_{\rm train}$ 线性相关——$n = 24{,}000$、$\mathrm{imgsz} = 224$、$\mathrm{batch} = 64$、200 epoch 的配置在单卡 RTX 3090 / 4090（24GB 显存）上约需 2--3 小时。若扩大至 $n = 10^5$，单次训练耗时增至 10--15 小时，整体实验周期将超出本研究可承担的计算预算。
...
\item \textbf{frame-level 随机划分}：以固定种子 \texttt{20260606} 对 Sewer-ML Train 的合格样本池做一次性无重复随机抽样，不引入任何启发式分组或分层，亦不做 per-inspection 帧数上限（后者在无 inspection\_id 时不可实现）。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{3.6cm}p{3.2cm}X}
        ...
        批大小 batch & 64                     & 五种容量在 24GB 显存（3090 / 4090）允许的统一上限 \\
        DataLoader workers & 4                & 共享内存与 RAM 在 24GB 主机的稳定值（worker $= 8$ 在多机实测中触发共享内存耗尽） \\
...
\subsection{训练产物的存储纪律与本地聚合}
\label{sec:artifact-discipline}
评估协议（图~\ref{fig:ch3-eval-pipeline}）的四个步骤均为\textbf{后训练计算}：温度拟合（$T^{\ast}$）、阈值搜索（$\tau_{R_0}$）、字典序选模（$\theta^{\ast}$）以及最终测试指标，本质上都是 logit 与标签的函数。这一观察驱动本文采取一条工程纪律——\textbf{只持久化"线性无关的原材料"，所有派生量均由本地脚本现算}。

\textbf{存储原则}。对每次训练 run（每容量 $\times$ 每种子）保留的内容仅限于"不重跑训练就无法重建"的最小集合：

\begin{itemize}[leftmargin=2em,nosep]
\item 200 个 epoch 检查点权重 \texttt{weights/epoch\{i\}.pt}；
\item 每个 epoch 的 val-cal 与 val-op 原始 logits、标签与 image\_id（\texttt{per\_epoch\_logits/val\_cal\_epoch\{i\}.npz} 与 \texttt{val\_op\_epoch\{i\}.npz}）；
\item 最优检查点 $\theta^{\ast}$ 在 test 上的原始 logits、标签与 image\_id（\texttt{best\_epoch/test\_logits.npz}）；
\item $\theta^{\ast}$ 在 train、val-op、test 上的倒数第二层特征（\texttt{best\_epoch/embeddings\_*.npz}），用于 RDTC 中的 $D$ 信号计算；
\item 训练元信息 \texttt{run\_meta.json}（git commit、host、GPU 型号、起止时间、总耗时），用于复现性审计。
\end{itemize}

明确\textbf{不持久化}的内容包括：每 epoch 的 $T^{\ast}$ 与 $\tau_{R_0}$、Spec/Prec/PTR/Recall 与 Wilson 区间、混淆矩阵、per-class recall、$\tau$-Spec 曲线、最优 epoch 指针、跨容量对比表——这些全部为 logits 与标签的确定性函数，由本地聚合脚本 \texttt{scripts/aggregate\_capacity\_results.py} 在分钟级时间内重算得到。

\textbf{设计动机}。该纪律的直接收益是\textbf{分析侧的实验自由度}：调整召回目标（$R_0 = 99.5\%$ 改为 $99\%$ 或 $98\%$）、调整字典序优先级、新增 ECE 或 Brier 等评价指标、提高块 bootstrap 重采样次数（$B = 2000$ 改为 $10000$），均无需重跑训练，仅需重新执行聚合脚本。这一解耦把"高代价的训练"与"低代价的分析"分离至两个独立环节，使后续章节的多视角对比与消融实验可以在不重新征用 GPU 资源的前提下迭代完成。冗余存储（如同时落盘 logits 和 $T^{\ast}$）从形式上无害，却引入"哪个数字是权威"的歧义并增加版本管理负担，本文一律去除。
