...
\subsection{采样流程}
本文采用随机采样（uniform random sampling）从采样池中抽取训练、验证与测试集。\textbf{采样种子固定}为 20260606，保证数据集构建可重现。采样过程为一次性完成：先抽取训练集所需样本数，再从剩余池中抽取验证集，最后从再剩余池中抽取测试集，三者逻辑上严格不重叠。该一次性采样流程相较于"先合并采样再划分"的做法具备两项优势：其一，避免划分边界上的数据泄漏风险；其二，便于在不同规模实验中扩展训练集而不影响既有验证与测试集的独立性。
