...
\section{论文组织}
全文围绕 stage-1 主线的 formal 选模、训练侧增强和样本重加权展开。第 2 章介绍高召回门控的理论基础、指标定义和样本效用建模动机；第 3 章说明数据集、实验平台和 formal working set 的组织方式；第 4 章报告 direct binary gate formal capacity scan 结果；第 5 章报告 six-class source 视图及其与 binary gate 的 cross-view mismatch；第 6 章报告主模型上的 HN ratio sweep 与 cross-capacity validation；第 7 章给出基于有效信息量的非线性样本重加权方法设计与 lite 消融框架；第 8 章总结当前已建立的 stage-1 证据链及其工程启示；第 9 章给出全文结论与后续研究方向。
...
\chapter{source-side 辅助视图与 cross-view 分析}
...
此外，本文当前尚未报告 Gate-Aligned Effective Information Score lite 的 full-run 结果。第 7 章给出的内容是建立在已完成 stage-1 证据基础上的方法设计与实验框架，而不是新的 completed performance claim。对于 split 尺度、整数步长对阈值定位的影响，以及更大规模跨设备数据条件下的稳定性问题，本文也仅在现有工作流范围内讨论，不对其外推结论作正式声明。
