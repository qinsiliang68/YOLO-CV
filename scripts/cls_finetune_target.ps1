param(
    [string]$Config = "YOLOv11/configs/runtime/cls_target_struct6.json",
    [string]$Data = "",
    [string]$Model = "",
    [string]$Device = "",
    [int]$Epochs = -1,
...
$args = @("run", "--project", $repoRoot, "--frozen", "--extra", $backend, "python", "scripts/cls_finetune_target.py", "--config", $Config)
