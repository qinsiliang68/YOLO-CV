...
DEFAULT_RUNS = [
    "yolo11n_cls6_train7200_uniform",
    "yolo11s_cls6_train7200_uniform",
    "yolo11m_cls6_train7200_uniform",
    "yolo11l_cls6_train7200_uniform",
    "yolo11x_cls6_train7200_uniform",
]
DEFAULT_OUTPUT_SUBDIR = "calibration_ts_cls6"
DEFAULT_SEED = 20260330
DEFAULT_CAL_FRACTION = 0.30
DEFAULT_BINS = 10
DEFAULT_EPS = 1e-6
SUMMARY_CSV_NAME = "stage1_cls6_calibration_current.csv"
SUMMARY_JSON_NAME = "stage1_cls6_calibration_current.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run temperature scaling on existing cls6 indirect-gate raw materials without retraining."
    )
    parser.add_argument(
        "--materials-root",
...
def short_model_name(run_name: str) -> str:
    return (
        run_name.replace("_cls6_train7200_uniform", "")
        .replace("_gate2_train7200", "")
        .replace("yolo11", "y11")
        .replace("-cls", "")
    )
...
def plot_reliability_all(results: list[dict[str, Any]], output_prefix: Path) -> None:
    ...
    fig.suptitle("Stage-1 cls6 indirect-gate reliability before and after calibration", fontsize=12)
...
def plot_threshold_sweep_all(results: list[dict[str, Any]], output_prefix: Path) -> None:
    ...
    if handles:
        fig.legend(handles, labels, loc="upper center", ncol=2, frameon=False)
    fig.suptitle("Stage-1 cls6 indirect-gate threshold sweep before and after calibration", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    save_figure(fig, output_prefix)
...
def plot_key_metric_summary(summary_rows: list[dict[str, Any]], output_prefix: Path) -> None:
    ...
    fig.suptitle("Stage-1 cls6 indirect-gate key operating-point metrics", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    save_figure(fig, output_prefix)
...
def run_single(
    ...
    *,
    ...
    summary_row = {
        "run_name": run_name,
        "model": run_name.replace("_cls6_train7200_uniform", "").replace("_train7200", "").replace("_gate2", ""),
    ...
    print_step(
        "done",
        (
            f"{run_name}: T={format_metric(summary_row['temperature'])}, "
            f"ECE {format_metric(summary_row['ece_before'])}->{format_metric(summary_row['ece_after'])}, "
            f"Spec@R99.0 {format_metric(summary_row['spec_r990_before'])}->{format_metric(summary_row['spec_r990_after'])}"
        ),
    )
    return {
        "run_name": run_name,
...
def main() -> None:
    ...
    plot_reliability_all(run_results, materials_root / "stage1_cls6_calibration_reliability_current")
    plot_threshold_sweep_all(run_results, materials_root / "stage1_cls6_calibration_sweep_current")
    plot_key_metric_summary(summary_rows, materials_root / "stage1_cls6_key_metrics_current")
    plot_reliability_all(run_results, essay_figure_dir / "stage1_cls6_calibration_reliability_current")
    plot_threshold_sweep_all(run_results, essay_figure_dir / "stage1_cls6_calibration_sweep_current")
    plot_key_metric_summary(summary_rows, essay_figure_dir / "stage1_cls6_key_metrics_current")
