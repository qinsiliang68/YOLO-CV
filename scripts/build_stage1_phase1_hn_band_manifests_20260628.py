...
def build_band_runs() -> list[BandRun]:
    ...
    for index in range(1, 21):
        start = index - 1
        runs.append(BandRun(f"HN1-{index:02d}", "HN1", index, start, start + 1))
    for index in range(1, 11):
        start = (index - 1) * 2
        runs.append(BandRun(f"HN2-{index:02d}", "HN2", index, start, start + 2))
    return runs
...
def add_band_fields(rows: list[dict[str, str]], band: BandRun, rank_start: int, rank_end: int) -> list[dict[str, str]]:
    out = []
    for row in rows:
        item = dict(row)
        item.update(
...
def build_normal_manifest_for_band(
    ...
    selected_rows_all = sorted_hn_rows[rank_start:rank_end]
    ...
    selection_rows = add_band_fields(selection_rows, band, rank_start, rank_end)
