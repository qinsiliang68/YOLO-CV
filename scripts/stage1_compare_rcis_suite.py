...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare stage-1 RCIS experiments under the shared PTSG protocol.")
    parser.add_argument("--baseline-dir", required=True, help="Baseline material directory.")
    parser.add_argument("--baseline-label", default="G4 current best HardMix + P0", help="Baseline row label.")
    parser.add_argument("--output-dir", required=True, help="Output directory for RCIS suite summary.")
    parser.add_argument(
        "--experiment",
        action="append",
        default=[],
...
def choose_best_variant(rows: list[dict[str, str]]) -> dict[str, str]:
...
def build_row(label: str, material_dir: Path) -> dict[str, Any]:
    ...
    best_row = choose_best_variant(summary_rows)
...
def main() -> None:
    ...
    verdict = "rcis_wins" if best["label"] != args.baseline_label else "baseline_still_best"

    write_csv(output_dir / "stage1_rcis_suite_summary.csv", rows)
    (output_dir / "stage1_rcis_suite_summary.json").write_text(
        json.dumps(
            {
    ...
    (output_dir / "stage1_rcis_suite_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
