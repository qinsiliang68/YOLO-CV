...
$runtimeConfig = Join-Path $repoRoot "YOLOv11\configs\runtime\stage1_formal_gate_hn_l_sweep.json"
$manifest = Join-Path $repoRoot "research\materials\stage1_formal\gate_capacity\yolo11l_gate2_formal\best_epoch_manifest.json"
...
Require-Path -Path $runtimeConfig -Label "l-sweep runtime config"
Require-Path -Path $manifest -Label "yolo11l base manifest"
...
Require-Path -Path $meta.checkpoint_path -Label "yolo11l base checkpoint"

Write-Host "[ok] repo root: $repoRoot"
Write-Host "[ok] yolo11l base checkpoint: $($meta.checkpoint_path)"
Write-Host "[ok] dataset: $datasetRoot"
Write-Host "[ok] split csv: $splitCsv"

$command = @("uv", "run", "main.py", "--task", "stage1_formal_gate_hn_l_sweep")
if ($DryRun) {
    $command += "--dry-run"
}
if ($Rerun) {
