...
Generated on 2026-06-17 from the final 1:5 normal evaluation manifests introduced in commit `d841de3`.

This is an evaluation-only rerun. No model weights were retrained. The trained `yolo11n/s/m/l/x` checkpoints were reused, while `val_cal`, `val_op`, and `test` were rerun with the corrected evaluation ratio:
...
- `node-192.168.100.18/eval_1to5_full_*/predictions_*.csv`: per-image prediction outputs for `val_cal`, `val_op`, and `test`.
...
All five model jobs completed with exit code `0`. Each prediction CSV has 120,001 lines including the header.
...
The old 1:1 evaluation artifacts remain in the repository for now but should not be used for final calibration, operating threshold selection, or final test comparison.
