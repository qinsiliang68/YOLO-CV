...
def _local_campaign_trainer(yolo_root: Path):
    ...
    module_name = f"{__package__}.campaign_ultralytics_overlay"
    # Re-import after activating the frozen YOLO tree so the overlay cannot retain
    # classes from a different Ultralytics installation during local review.
    sys.modules.pop(module_name, None)
    overlay = importlib.import_module(module_name)
    return overlay.RelocatableCampaignClassificationTrainer
