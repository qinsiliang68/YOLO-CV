# Appendix Table: Stage-1 Capacity Protocol

| Item | Formal Rule |
| --- | --- |
| Primary task | Direct binary gate (Normal / Abnormal) |
| Auxiliary task | Six-class source capacity scan |
| Batch size | 24 |
| Epochs | 200 |
| Checkpoint policy | Save every epoch |
| Early stopping | Disabled during formal capacity scan |
| Binary gate ranking | Spec@R99.5 > Spec@R99.0 > Prec@R99.0 > PTR@R99.0 |
| CLS6 ranking | Accuracy > AUROC > AUPRC |
| Calibration protocol | Temperature fit on val-cal, evaluate on val-op |
| Trainer top1 role | Training-health only, not the formal stage-1 selection rule |
