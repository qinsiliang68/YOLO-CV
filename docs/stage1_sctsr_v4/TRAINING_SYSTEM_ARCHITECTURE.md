...
T/R1/R2 是离线生成并哈希绑定的固定 identity pool。R2 的白名单投影发生在 matcher 之前，且 quota key 是 `(label, historical_dynamic_bucket, oof_fold, oof_group_id)`。五个 identity group 由 ID 稳定散列产生，不读取原 rank。
