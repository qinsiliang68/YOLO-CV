...
import stage1_gapvalue240.campaign_worker as campaign_worker
from stage1_gapvalue240.campaign_layout import ACTIVE_RUN_QUEUE_DIRNAME
...
def test_formal_worker_loads_the_active_v2_queue(
    tmp_path: Path,
    ...
    class FakeMachine:
        data = {"dry_run": False, "num_workers": 4}

    class StopAfterQueueLookup(RuntimeError):
        pass

    def capture_release(queue_dir: Path, *_args, **_kwargs):
        observed["queue_dir"] = Path(queue_dir)
        raise StopAfterQueueLookup

    monkeypatch.setattr(campaign_worker, "load_machine_config", lambda _path: FakeMachine())
    monkeypatch.setattr(campaign_worker, "load_campaign_release", capture_release)

    with pytest.raises(StopAfterQueueLookup):
        campaign_worker.run_campaign_job(
            tmp_path / "machine.json",
            campaign,
            "JOB_ONE",
            release_path=tmp_path / "release.json",
            assignment_path=tmp_path / "assignment.json",
            expected_release_id="PILOT",
            expected_canonical_lock_sha256="A" * 64,
        )

    assert observed["queue_dir"] == campaign.resolve() / ACTIVE_RUN_QUEUE_DIRNAME
