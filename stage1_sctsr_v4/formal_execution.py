...
def claim_formal_execution(
    ...
    *,
    ...
    claimed_at = (now_utc or datetime.now(timezone.utc)).astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
    core = {
        "schema_version": "stage1.sctsr.formal_execution_claim.v1",
        "status": "CLAIMED",
    ...
    if hasattr(os, "O_BINARY"):
        flags |= os.O_BINARY
    try:
        claim_fd = os.open(claim_path, flags, 0o444)
    ...
    try:
        ...
        while offset < len(encoded):
            written = os.write(claim_fd, encoded[offset:])
            if written <= 0:
                raise OSError("exclusive execution claim write made no progress")
            offset += written
        os.fsync(claim_fd)
    finally:
        os.close(claim_fd)
...
def validate_execution_claim_binding(
    ...
    *,
    ...
    if (
        claim.get("schema_version") != "stage1.sctsr.formal_execution_claim.v1"
...
def publish_execution_claim_snapshot(
    ...
    *,
    ...
    core = {
        "schema_version": "stage1.sctsr.execution_attempt_snapshot.v1",
...
def validate_execution_attempt_snapshot(
    ...
    *,
    ...
    if (
        snapshot.get("schema_version") != "stage1.sctsr.execution_attempt_snapshot.v1"
    ...
    if (
        ...
        or claim.get("schema_version") != "stage1.sctsr.formal_execution_claim.v1"
        or claim.get("status") != "CLAIMED"
